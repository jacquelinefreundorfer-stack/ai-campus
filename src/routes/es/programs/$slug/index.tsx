...
export const Route = createFileRoute("/es/programs/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!bundle) return { meta: [{ title: "Programa — AI Campus" }] };

    const ogTags = buildOgTags({
      title: `${bundle.title} — AI Campus`,
      description: bundle.description || "",
      url: `${SITE_URL}/es/programs/${bundle.slug}`,
      type: "website",
    });
...
function BundleDetailPage() {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      setError(e.message || "Error al inscribirse. Inténtelo de nuevo.");
      setEnrolling(false);
    }
  };
    ...
    <div className="min-h-dvh bg-cream">
      <div className="bg-navy px-6 py-16 text-white">
        <div className="max-w-5xl mx-auto">
          <Link to="/es/programs" className="text-gold/60 hover:text-gold text-sm mb-6 inline-block">← Todos los programas</Link>
          ...
          <div className="flex flex-wrap gap-4 mt-6 text-sm text-gray-400"><span>{bundle.modulesCount} módulos</span><span>·</span><span>~{bundle.hours} horas</span></div>
        </div>
      </div>
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <h2 className="font-serif text-2xl font-bold text-navy mb-4">Acerca de este programa</h2>
            <p className="text-gray-600 leading-relaxed mb-10">{bundle.description}</p>
            <h2 className="font-serif text-2xl font-bold text-navy mb-6">Plan de estudios</h2>
            <div className="space-y-4">
              {modules.map((mod: any) => (
                <div key={mod.id} className="bg-white border border-gray-200 p-5">
                  <h3 className="font-serif text-lg font-semibold text-navy">Módulo {mod.sortOrder}: {mod.title}</h3>
                  {mod.description && <p className="text-sm text-gray-500 mt-1">{mod.description}</p>}
                  <div className="mt-2 text-xs text-gray-400">{mod.lessons.length} lecciones{mod.quizId && <span> · Cuestionario incluido</span>}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="bg-white border border-gray-200 p-6 sticky top-6">
              ...
              {enrollmentId ? (
                <div className="space-y-3">
                  <p className="text-green-700 font-medium text-sm">✓ ¡Inscrito!</p>
                  <Link
                ...
                <div className="space-y-3">
                  <p className="text-sm text-gray-500 text-center">Inicie sesión para inscribirse.</p>
                  <button
                    onClick={() => {
                  ...
                  <p className="text-xs text-gray-400 text-center">Cohorte beta: inscripción gratuita</p>
                </div>
              ) : (
                <div className="space-y-3">
